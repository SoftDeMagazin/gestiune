...
function lista($frmFiltre=array(), $frmPager=array(), $action="first", $selected=0)
{
	...
	$model = new UnitatiMasura("where denumire like '%". $frmFiltre['denumire'] ."%' order by denumire asc");
	}
	else {
	$model = new UnitatiMasura("where 1 order by denumire asc");
	}
	...
	$objResponse -> assign("grid", "innerHTML", $info['page'] -> lista("", "xajax_frm('<%unitate_masura_id%>')", $selected));
...
function frm($id=0)
{
	$model = new UnitatiMasura($id);
	$out = $model -> frmDefault();
	$objResponse = new xajaxResponse();
	$objResponse -> assign("frm", "innerHTML", $out);
	$btn = '
	 <div align="right">
   <input type="submit" name="btnSave" id="btnSave" value="Salveaza" onClick="xajax_save(xajax.getFormValues(\'frm_unitati_masura\'), xajax.getFormValues(\'frmFiltre\'),xajax.getFormValues(\'frmPager\'))" tabindex="6">
...
{
	$model = new UnitatiMasura($frmValues);
	$objResponse = new xajaxResponse();
