...
function load($filtre) {
	...
	if($filtre['gestiune_id']) {
		...
		$sql .= " and plati_efecte.gestiune_id in (". $in .")";
	}		
	
	if($filtre['tert_id']) {
		$in = "'".implode("','", $f['tert_id'])."'";
		$sql .= " and platiEfecte.tert_id in (". $in .")";
	}		
	$efecte = new PlatiEfecte("where operat = 'NU'");
	$objResponse = new xajaxResponse();
	$objResponse -> assign("grid", "innerHTML", $efecte -> listaRptNeplatite());
