 ...
 */
class PrintAvizPentruFacturaPdf extends PdfHelper {
	...
	function __construct($aviz_id) {
		...
		$this -> factura = new Facturi($this -> aviz -> factura_id);
	...
	function infoClient() {
		...
		if($this -> factura -> transfer_id > 0) {
			...
			$this -> text($societate -> denumire ." / ". $gestiune -> denumire, $left, 53, 12);
			$this -> text($societate -> reg_com, $left, 57);
			$this -> text($societate -> cod_fiscal, $left, 61, 10, 'N'); 
			$this -> text($societate -> sediul.' '.$this -> tert -> cod_tara, $left,65, 10);
			$this -> text($societate -> banca, $left, 69);
			$this -> text($societate -> iban, $left, 73);
		} else {
	...
	function infoDoc($pagina, $din) {
		...
		$this -> text("Factura", 3, 66);
		$this -> text(str_pad($this -> factura -> numar_doc, 9, "0", STR_PAD_LEFT), 55, 66);
	...
	function detaliiFactura() {
		$y = 88;
		$this -> text("Moneda: ". $this -> factura -> valuta, 5, $y);
		$this -> text("Curs EUR = ". $this -> factura -> curs_valutar  ." RON", 50, $y);
		$this -> text("Incoterms: ".$this -> factura -> incoterm, 110, $y);
		$this -> text("Cota TVA:".$this -> factura -> cota_tva -> valoare, 160, $y);
	}
	/**
	...
	function antetContinutFactura() {
		...
		$this -> pdf -> Rect(170, $y, 39,10); //valoare
