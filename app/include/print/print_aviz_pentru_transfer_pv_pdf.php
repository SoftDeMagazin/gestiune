 ...
 */
class PrintAvizPentruTransferPvPdf extends PdfHelper {
	...
	function __construct($aviz_id) {
		...
		switch($this -> aviz -> tip_aviz) {
			case "la_transfer": {
				$transfer = $this -> aviz -> transfer;
				$this -> transfer = $transfer;
				$this -> societate = $transfer -> gestiune -> societate;
				$this -> tert = new Terti($transfer -> gestiune_destinatie -> tert_id);
			}break;
		}
		$this -> pdfProperties();
	}
	...
	function infoSocietate() {
		...
		$this -> text("GESTIUNE: ". $this -> transfer -> gestiune -> denumire, $left,41, 10);
	}
	/**
	 * informatii client
	 * @return 
	...
	function infoClient() {
		...
		$this -> text("GESTIUNE: ". $this -> transfer -> gestiune_destinatie -> denumire, $left, 77);
	...
	function infoDoc($pagina, $din) {
		...
		$this -> text("AVIZ DE EXPEDITIE", 16, 50, 14, 'B');
		$this -> text("Numar", 3, 55);
		$this -> text(str_pad($this -> aviz -> numar_doc, 9, "0", STR_PAD_LEFT), 55, 55, 12, 'B');
		$this -> text("Data", 3, 61);
		$this -> text(c_data($this -> aviz -> data_doc), 55, 61, 12, 'B');
	...
	function antetContinutFactura() {
		...
		$this -> text("Pret Fara TVA", 151, $y+4,10,'B');
		
		$this -> text("Valoare Fara TVA", 181, $y+4,10,'B');
	...
	function articol($nr_crt, $cnt, $y) {
		...
		$this -> text($cnt['denumire'], 16, $y, 9, 'N');
		...
		$this -> pdf -> writeHTMLCell(29, 13, 151, $y - 4, '<p style="text-align:right;">'. $cnt['pret_vanzare_ron'] .'</p>');
		$this -> pdf -> writeHTMLCell(28, 13, 181, $y - 4, '<p style="text-align:right;">'. $cnt['val_vanzare_ron'] .'</p>');
	...
	function total() {
		...
		$total_aviz = number_format($this -> aviz -> totalAvizPvLei(), 2, ',', '.');
		$this -> text("Total", 142, 266, 10, 'B');
		$this -> pdf -> SetFontSize(14);
		$this -> pdf -> writeHTMLCell(36, 13, 174, 260, '<p style="text-align:center;">'. $total_aviz .'</p>');
	}
	...
	function continutFactura() {
		...
		$sql = "
				select
				avize_continut.produs_id,
				produse.denumire,
				produse.unitate_masura_id,
				sum(avize_continut.cantitate) as cantitate,
				avize_continut.pret_vanzare_ron,
				sum(avize_continut.cantitate * avize_continut.pret_vanzare_ron) as val_vanzare_ron
				from avize_continut
				inner join produse on avize_continut.produs_id = produse.produs_id
				where avize_continut.aviz_id = '". $this -> aviz -> id ."'
				group by avize_continut.produs_id, produse.denumire 
				";
	...
	function intocmit() {
		$x = 10;
		$y = 244;
		$user = new Utilizatori($this -> aviz -> utilizator_id);
