...
class Plati extends Model
{
	...
	function lista($click="", $dblClick="" , $selected=0)
	{
		...
		for($i=0;$i<$nr_r;$i++)
			...
			$dg -> addColumn(money(douazecimale($this -> suma), $this -> tert -> valuta));
			$dg -> addColumn(money(douazecimale($this -> sumaAsociata()), $this -> tert -> valuta));
