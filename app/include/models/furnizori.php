...
class Furnizori extends Model
{
	...
	var $_validator = array(
		"denumire" => array(array("required", "Introduceti denumire"), array("unique", "Denumire existenta")),
	);	
	...
	{
		$nr_r = count($this);
		$out = '<select name="furnizor_id" id="furnizor_id" style="width:400px">';
		$out .= '<option value="0">Selectare Furnizor</option>';	
		for($i=0;$i<$nr_r;$i++) {
			$this -> fromDataSource($i);
