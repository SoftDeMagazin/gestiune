<?php
class RptEfectePlatite extends Rpt {
	...
	function getConditions() {
		...
		if($f['gestiune_id']) {
			...
			$conditions .= " and plati_efecte.gestiune_id in (". $in .")";
		}		
		
		if($f['tert_id']) {
			$in = "'".implode("','", $f['tert_id'])."'";
			$conditions .= " and plati_efecte.tert_id in (". $in .")";
		}		
		return $conditions;
	}
	function getAntet() {
		$out .= '<h2 align="center">EFECTE DE COMERT PLATITE</h2>';
	...
	function getHtml() {
		...
		$dg -> addHeadColumn("Data Incasare");
		...
		for($i=0;$i<$nr_r;$i++) {
			...
			$dg -> addColumn(c_data($row['data_incasare']));
			$dg -> addColumn($row['suma']);
			$dg -> addColumn($row['suma_acceptata']);
			switch($row['raspuns']) {
				case "OK": {
			...
			$dg -> index();
			$total_emis += $row['suma'];
			$total_acceptat += $row['suma_acceptata'];
			...
			if($row['gestiune'] != $this -> data[$i+1]['gestiune']) {
				...
				$dg -> addColumn($total_acceptat);
				$dg -> addColumn(douazecimale(($total_acceptat/$total_emis)*100)."%");
				$out .= $dg -> getDataGrid();
				$total_emis = 0;
				$total_acceptat = 0;
			}
