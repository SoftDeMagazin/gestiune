<?php
class RptBalantaStocuri extends Rpt {
	...
	function getAntet() {
		$out .= '<h2 align="center">Balanta Stocuri Cantitativa</h2>';
	...
	function getHtml() {
		...
		$dg -> addHeadColumn("Stoc Initial", array("width" => "15%"));
		$dg -> addHeadColumn("Intrari", array("width" => "15%"));
		$dg -> addHeadColumn("Iesiri", array("width" => "15%"));
		$dg -> addHeadColumn("Stoc Final", array("width" => "15%"));
		...
		for($i=0;$i<$nr_r;$i++) {
			...
			$dg -> addColumn(treizecimale($stoc_final), array("width" => "15%", "align" => "center"));
			$dg -> index();
